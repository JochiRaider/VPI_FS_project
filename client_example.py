...
def create_client_config(module):
    global client_id
    client_id = f'ent-{int(time.time())}'
    task_queue.put(1)
    result = sys.modules[module].run()
    task_queue.get()
    repo = connect_to_github()
    remote_path = f"config/{client_id}.json"
    message = f'{client_id} init {datetime.now().isoformat()}' 
...
def main():
    ...
    while True:
        if task_queue.empty():
            ...
            for task in config:
                if task['module'] == 'rookie':
                    t = threading.Thread(target=create_client_config , args=(task['module'],))
                    t.start()
